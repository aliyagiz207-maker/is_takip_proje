...
namespace is_takip_proje.Formlar
{
    public partial class FrmPersonelİstatistik : Form
    {
        ...
        private void FrmPersonelİstatistik_Load(object sender, EventArgs e)
        {
            ...
            LblBugunkuGorevler.Text = db.TblGorevler.Count(x => x.Tarih == DateTime.Today).ToString();
            LblBugunkuIsSayisi.Text = db.TblGorevler.Count(x => x.Tarih == DateTime.Today && x.Durum == true).ToString();
            var d1 = db.TblGorevler.GroupBy(x => x.GorevAlan).OrderByDescending(z => z.Count()).Select
            (y => y.Key).FirstOrDefault();
            LblAyinPersoneli.Text = db.TblPersonel.Where(x => x.ID == d1).Select(y => y.Ad + " " + y.Soyad).FirstOrDefault();
            LblAyinDepartmani.Text = db.TblDepartmanlar.Where(x => x.ID == db.TblPersonel.Where(z => z.ID == d1).FirstOrDefault().ID).Select(y => y.Ad).FirstOrDefault().ToString();
        }
    }
}
